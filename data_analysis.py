...
def accuracy_calculation(game_name):
    ...
    for i in range(0,len(game_data)):
        ...
        accuracy_list[dif][2] = accuracy_list[dif][0]/accuracy_list[dif][1]
    ...
    general_accuracy_list[2] = general_accuracy_list[0]/general_accuracy_list[1]
        ...
        if game!='Math':
            if data['threshold'][game]!=[]:
                ...
                for dif in accuracy_thr[subject_id][game].keys():
                    ...
                    outrow = {'Type':'threshold','Subject_ID':subject_id,'Difficulty':dif,'Total_Score':score,'Total_Trial':accuracy_thr[subject_id][game][dif][1],'Accuracy':accuracy_thr[subject_id][game][dif][2]}
                ...
                outrow_sum= {'Type':'threshold','Subject_ID':subject_id,'Game':game,'Difficulty':dif,'Total_Score':score_sum,'Total_Trial':general_accuracy_thr[subject_id][game][1],'Accuracy':general_accuracy_thr[subject_id][game][2]}
                write_to_file('Summary',outrow_sum,'no_extra_row')

                #create list of accuracy only for all games
                gen_accuracy.append(general_accuracy_thr[subject_id][game][2])
        
            else:  print(subject_id,game,'skipped')

    ##calculating and writing outputs for mean_perID
    if gen_accuracy!=[]:
        mean_perID = sum(gen_accuracy)/len(gen_accuracy)
        mean_all.append(mean_perID) #appending mean_perID to a list
        outrow_perID = {'Type':'threshold','Subject_ID':subject_id,'Accuracy':mean_perID}
...
if mean_all!=[]:
    mean_allID = sum(mean_all)/len(mean_all)
    outrow_perID = {'Type':'threshold','Subject_ID':'Mean_all','Accuracy':mean_allID}
    write_to_file('Mean_perID',outrow_perID,'no_extra_row')
...
for game in games:
    ...
    if game!='Math':
        ...
        for dif in accuracy_byTask[game].keys():
            accuracy_task = sum(accuracy_byTask[game][dif])/len(accuracy_byTask[game][dif])
            mean_task[game].append(accuracy_task)
            outrow_byTask = {'Type':'threshold','Game':game,'Difficulty':dif,'Accuracy':accuracy_task}
            write_to_file('Mean_byTask',outrow_byTask,'no_extra_row')

        ##calculate overall mean accuracy byTask
        if mean_task[game]!=[]:
            mean_task_value = sum(mean_task[game])/len(mean_task[game])
            outrow_byTask = {'Type':'threshold','Game':game+'_mean','Difficulty':'','Accuracy':mean_task_value}
            write_to_file('Mean_byTask',outrow_byTask,'no_extra_row')
