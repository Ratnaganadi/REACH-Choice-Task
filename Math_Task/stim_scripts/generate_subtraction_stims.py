...
def count_carries(exp):
    if '+' in exp:
        ...
        base=10
        lhs = exp.split()[0]; lhs = lhs.replace('.','')
        lhs = [int(lhs[i]) for i in range(len(lhs))]
        rhs = exp.split()[-1]; rhs = rhs.replace('.','')
        rhs = [int(rhs[i]) for i in range(len(rhs))]
...
out_file= open('../math_stims_subtraction.csv', 'w')
...
# 2 levels of subtraction, no carrying
for i, num_range in enumerate([range(1,5), range(1,10)]):
    ...
    create_row(all_expressions=all_exps, difficulty=difficulty, max_stims=16, label='subtraction range %d-%d, no carrying'%(num_range[0],num_range[-1]))
    difficulty+=1

# 1 level of two-digit and one-digit subtraction, no carrying
all_exps = create_product_expressions(num_set1=range(1,10), num_set2=range(11,20), operators=['-'])
all_exps = separate_carrying(all_exps)[1] # get expressions without carrying
create_row(all_expressions=all_exps, difficulty=difficulty, max_stims=16, label='subtraction two-digit and one-digit, no carrying')
difficulty+=1

# 1 level of one-digit subtraction, with carrying
#all_exps = create_expressions(num_range=range(1,10), operators=['-'])
#all_exps = separate_carrying(all_exps)[0] # get expressions with carrying
#create_row(all_expressions=all_exps, difficulty=difficulty, max_stims=16, label='subtraction range 1-9, with carrying')
#difficulty+=1

# 1 level of two-digit and one-digit subtraction, with carrying
all_exps = create_product_expressions(num_set1=range(1,10), num_set2=range(11,20), operators=['-'])
all_exps = separate_carrying(all_exps)[0] # get expressions with carrying
create_row(all_expressions=all_exps, difficulty=difficulty, max_stims=16, label='subtraction two-digit and one-digit, with carrying')
difficulty+=1

# 1 level of subtraction with multiples of 10 to one-digit
all_exps = create_product_expressions(num_set1=range(1,10), num_set2=list(drange(10,91,10)), operators=['-'])
all_exps = ['{} - {}'.format(int(exp.split()[0]) + int(exp.split()[-1]), exp.split()[0]) for exp in all_exps]
create_row(all_expressions=all_exps, difficulty=difficulty, max_stims=16, label='subtraction one-digit to multiples of 10')
difficulty+=1

# subtraction 2-digit, no carrying
all_expressions = create_expressions(num_range=range(11,100), operators=['-'])
all_expressions = separate_carrying(all_expressions)[1]
create_row(all_expressions=all_expressions, difficulty=difficulty, max_stims=16, label='subtraction 2-digit, no carrying')
difficulty+=1

# Decimal subtraction, no carrying
num_range = []
for root in xrange(1,10):
    num_range.extend(list(drange(root+0.1, root+0.9, 0.1)))
all_exps = create_expressions(num_range=num_range, operators=['-'])
all_exps = separate_carrying(all_exps)[1]
create_row(all_expressions=all_exps, difficulty=difficulty, max_stims=16, label='Decimal subtraction range %d-%d, no carrying'%(num_range[0],num_range[-1]))
difficulty+=1

# subtraction 2-digit, with carrying
all_expressions = create_expressions(num_range=range(11,100), operators=['-'])
all_expressions = separate_carrying(all_expressions)[0]
create_row(all_expressions=all_expressions, difficulty=difficulty, max_stims=16, label='subtraction 2-digit, with carrying')
difficulty+=1

# Decimal subtraction, with carrying
num_range = []
for root in xrange(1,10):
    num_range.extend(list(drange(root+0.1, root+0.9, 0.1)))
all_exps = create_expressions(num_range=num_range, operators=['-'])
all_exps = separate_carrying(all_exps)[0]
create_row(all_expressions=all_exps, difficulty=difficulty, max_stims=16, label='Decimal subtraction range %d-%d, with carrying'%(num_range[0],num_range[-1]))
difficulty+=1

# 3-digit to 2-digit subtraction
all_expressions = create_product_expressions(num_set1=range(13,100), num_set2=range(101,1000), operators=['-'])
all_expressions = separate_carrying(all_expressions)[0]
create_row(all_expressions=all_expressions, difficulty=difficulty, max_stims=16, label='3-digit to 2-digit subtraction')
difficulty+=1